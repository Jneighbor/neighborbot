...
class Owner:
    ...
    async def _reload(self, *, cog_name: str):
        ...
        try:
            ...
            log.exception(e)
            traceback.print_exc()
            await self.bot.say("```Failed to Reload Module! Check Development logs```")
        else:
            set_cog(module, True)
            await self.disable_commands()
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets RBOTed's prefixes for this server
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart Neighbor's Modules"
        ...
        try:
            if not silently:
                await self.bot.say(" ```Attempting to Restart! Please give me 30 seconds  ```")
        except:
            pass
    ...
    @checks.is_owner()
    async def join(self):
        """This gives a OAUTH2 Link to invite Neighbor to a new server"""
    ...
    async def contact(self, ctx, *, message : str):
        ...
        embed.add_field(name="About Neighbor", value=about, inline=False)

...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        ...
        The launcher automatically restarts bot when that happens"""
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Neighbor - First run configuration\n")
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("The owner module is missing. It contains core functions without "
              "which Neighbor cannot function. Reinstall.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial modules loading (--no-cogs)")

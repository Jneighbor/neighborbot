...
class Downloader(threading.Thread):
    ...
    def run(self):
        try:
            ...
            log.warning("URL Failed to Download '{}':\n'{}'".format(self.url, str(e)))
        self.done.set()
...
class Audio:
    ...
    async def audioset_cachemax(self, size: int):
        ...
        if size < self._cache_min():
            await self.bot.say("Sorry, I cannot safely allow"
    ...
    async def audioset_timerdisconnect(self, ctx):
        ...
        if not timer_disconnect:
            await self.bot.say("The bot will automatically disconnect after"
                               ...
                               " bot from disconnecting with other music modules"
                               " playing.")
        else:
            await self.bot.say("The bot will no longer auto disconnect"
                               " while other music modules are playing.")
        self.save_settings()
    ...
    async def cache_stats(self):
        """Reports info about the cache.
            ...
            - Minimum cache size. Automatically determined by number of servers Neighbor is running on.
        """
    ...
    async def play_local(self, ctx, *, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say("Please give me join perms.")
                return
            except UnauthorizedSpeak:
                await self.bot.say("I am unable to use voice modules in this channel")
                return
        ...
        if self.is_playing(server):
            await self.bot.say("I'm already playing a song??")
        ...
        if self.currently_downloading(server):
            await self.bot.say("```File Download in Progress```")
            return
    ...
    async def pause(self, ctx):
        ...
        if not hasattr(voice_client, 'audio_player'):
            await self.bot.say("ERROR: Nothing is playing, unable to pause")
    ...
    async def play(self, ctx, *, url_or_search_terms):
        ...
        try:
            ...
            await self.bot.say("Sorry, I cannot play anything if you are not in a voice channel.")
            return
        except UnauthorizedConnect:
            await self.bot.say("It seems I lack the permissions to join said channel.")
            return
        except UnauthorizedSpeak:
            await self.bot.say("I lack permissions to use the voice modules.")
            return
        except ChannelUserLimit:
            await self.bot.say("Your voice channel is full.")
            return
        ...
        if self.currently_downloading(server):
            await self.bot.say("```File Download in Progress, please wait.```")
            return
    ...
    async def sing(self, ctx):
        """Makes Bot sing one of their songs"""

...
DISCLAIMER = ("You're about to add a 3rd party repository. ")
...
class Downloader:
    ...
    async def info(self, repo_name: str, cog: str=None):
        """Shows info about the specified modules"""
        if cog is not None:
            ...
            if cog in cogs:
                ...
                if data:
                    ...
                    await self.bot.say("The specified modules has no info file.")
            else:
                await self.bot.say("That modules doesn't exist."
                                   " Use modules list to see the full list.")
        else:
            data = self.get_info_data(repo_name)
            if data is None:
    ...
    async def _install(self, ctx, repo_name: str, cog: str):
        ...
        if install_cog:
            ...
            await self.bot.say("That module doesn't exist. Use modules list to see"
                               " the full list.")
    ...
    def update_repo(self, name):
        ...
        try:
            ...
            # Make sure we don't git reset the Neighbor folder on accident
